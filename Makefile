...
FUSES      = -U hfuse:w:0xd9:m -U lfuse:w:0x3f:m
# ATTiny13 fuse bits (fuse bits for other devices are different!):
# As with all fuses, 0 if programmed
# Example for 4 MHz internal oscillator
# Fuse high byte:
# 0xdc = - - - 1   1 1 1 0 <-- RSTDISBL External Reset disable
...
#        +-------------------- Not Used
# Fuse low byte:
# 0x62 = 0 1 1 1   1 0 1 0
#        ^ ^ ^ ^   \/  \/
#        | | | |    |   +----- CKSEL 1..0 (9.6Mhz internal RC)
...
AVRDUDE_PORT = /dev/cu.usbserial-A20e1qOi    # programmer connected to serial device
#AVRDUDE_PORT = /dev/cu.usbserial5    # programmer connected to avr910
